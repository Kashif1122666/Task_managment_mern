...
{statCard({ title: 'Completion Rate', value: `${stats.completionPercentage}%`, icon: <Zap className='w-4 h-4 text-purple-500' /> })}
                  ...
                  <div className='bg-white rounded-xl py-4 sm:p-5 shadow-sm border border-purple-100'>
                         <h3 className='text-base sm:text-lg font-semibold mb-3 sm:mb-4 text-gray-800 flex items-center gap-2'>
                             <Clock className='w-4 h-4 sm:w-4 sm:h-5  text-purple-500' />
                             Recent Activities
                         </h3>
                         <div className=' space-y-2 sm:space-y-3'>
                             {tasks.slice(0,3).map((task)=>(
                                <div key={task._id || task.id} className='flex items-center justify-between p-2
                                sm:p-3 hover:bg-purple-50/50 rounded-lg transition-colors duration-200 border border-transparent hover:border-purple-100'>
