...
const Login = ({onSubmit ,onSwitchMode}) => {
  ...
  const handleSubmit = async (e) =>{
      e.preventDefault();
      if(!rememberMe) {
            toast.error('you must enalbe "Remember Me" to login.')
            return
      }
      setLoading(true)
      try {
  ...
  return (
    <div className='max-w-md bg-white w-full shadow-lg border border-purple-100 rounded-xl p-8'>
          ...
          <div className='mb-6 text-center'>
            ...
            <h2 className='text-sm mt-1 text-gray-500'>Sign in to continue to Task Manager</h2>
          </div>
          ...
          <p className='text-center text-sm text-gray-600 mt-6'>
               ...
               <button onClick={handleSwitchMode} className='text-purple-600 hover:text-purple-700 hover:underline font-medium transition-colors' type='button'>
                    Sign up
               </button>
          </p>
    </div>
